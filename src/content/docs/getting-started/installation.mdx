...
<Callout type="info">
  `@zengrid/core` includes runtime dependencies such as `@zengrid/shared`. Let your package manager resolve transitive dependencies from npm.
</Callout>
...
grid.setData(
  Array.from({ length: 1000 }, (_, row) =>
    Array.from({ length: 10 }, (_, col) => `Row ${row}, Col ${col}`)
  )
);
grid.render();
